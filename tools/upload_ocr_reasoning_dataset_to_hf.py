...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sube el dataset OCR razonador a Hugging Face Hub.")
    ...
    token = get_token()
    if not token:
        raise RuntimeError("No se encontro token Hugging Face. Inicia sesion antes de subir.")
    ...
    api.upload_folder(
        ...
        commit_message="Upload corrected math OCR reasoning dataset",
    )
